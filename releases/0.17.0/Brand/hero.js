// A finite wordmark light reveal. The scene never moves; the original image takes over at rest.
(() => {
 ...
 const usable=()=>ready&&!disposed&&!document.hidden&&!holder.hidden&&!reduced()&&(manual!==null||time<1.65);
 ...
 function tick(now){raf=0;if(!usable()){holder.classList.remove('motion-ready');return;}holder.classList.add('motion-ready');if(now>=nextFrame){draw(manual??Math.min(1.65,(now-start)/1000));nextFrame+=1000/60;if(nextFrame<=now)nextFrame=now+1000/60;}if(time>=1.65&&manual===null){holder.classList.remove('motion-ready');stop();return;}raf=requestAnimationFrame(tick);}
 ...
 window.highlineHero={get time(){return time;},get reduced(){return reduced();},state:()=>({ready,active:usable(),disposed,time,frames,width:canvas.width,height:canvas.height,kind:'wordmark-only',settled:time>=1.65}),seek(t){manual=t;draw(t);sync();},resume(){manual=null;start=performance.now()-time*1000;sync();},dispose};

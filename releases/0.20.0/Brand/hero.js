// A finite carving highlight along the wordmark relief and swash. The scene never moves; the original image takes over at rest.
(() => {
 ...
 const usable=()=>ready&&!disposed&&!document.hidden&&!holder.hidden&&!reduced()&&(manual!==null||time<2.6);
 ...
 function tick(now){raf=0;if(!usable()){holder.classList.remove('motion-ready');return;}holder.classList.add('motion-ready');if(now>=nextFrame){draw(manual??Math.min(2.6,(now-start)/1000));nextFrame+=1000/60;if(nextFrame<=now)nextFrame=now+1000/60;}if(time>=2.6&&manual===null){holder.classList.remove('motion-ready');stop();return;}raf=requestAnimationFrame(tick);}
 ...
 window.highlineHero={get time(){return time;},get reduced(){return reduced();},state:()=>({ready,active:usable(),disposed,time,frames,width:canvas.width,height:canvas.height,kind:'wordmark-only',duration:2.6,settled:time>=2.6}),seek(t){manual=t;draw(t);sync();},resume(){manual=null;start=performance.now()-time*1000;sync();},dispose};
 ...
 async function initialize(){
  ...
  const [fragment,...images]=await Promise.all([fetch('Brand/hero.frag').then(r=>{if(!r.ok)throw Error('Motion shader unavailable');return r.text();}),...['highline.png','highline-plate.png','hero-masks.png'].map(src=>new Promise((resolve,reject)=>{const i=new Image();i.onload=()=>resolve(i);i.onerror=reject;i.src='Brand/'+src;}))]);
